...
export const ENDPOINTS = {
  ...
  listDataAgreements: (offsetValue:number, pageSize: number, filter:string) => { return `/config/data-agreements?limit=${pageSize}&offset=${offsetValue}${filter === 'complete' ? '&lifecycle=complete' : ''}`},
  addDataAgreements: () => { return '/config/data-agreement'},
  listDataAttributes: (offsetValue:number, pageSize:number, filter:string) => { return `/config/data-agreements/data-attributes?limit=${pageSize}&offset=${offsetValue}${filter !== 'all' ? `&methodOfUse=${filter}` : ''}`},
  ...
  listAllAdminLogs:(offsetValue:number, pageSize:number,filter:number)=>{ return `/audit/admin/logs?limit=${pageSize}&offset=${offsetValue}${filter !== 0 ? `&logType=${filter}` : ''}`},
  listAllDataAgreementRecords: (offsetValue: number, pageSize: number,filter:any) => { return `/audit/data-agreement-records?limit=${pageSize}&offset=${offsetValue}${filter.filterType !== 'all' ? `&${filter.filterType}=${filter.value}` : ''}`},
